...
3. Rebuilds the backend (swapping in the new binary only on a successful build,
   keeping `server.bak`) and rebuilds the frontend.
4. Restarts the services and **health-checks** the new build at `/api/health`.
5. If the new build doesn't come up, it **automatically restores** the previous
   binary and git ref and restarts — so a bad release rolls itself back.
