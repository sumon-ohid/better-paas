...
| `paas_deploy` | Deploy from Git |
| `paas_redeploy` | Rebuild an existing app |
| `paas_get_logs` | Runtime logs |
| `paas_status` | Connection + app summary |

The MCP server reads `~/.paas/config.json` — no extra env vars needed.
...
HTTP client — curl, GitHub Actions, custom scripts, or an MCP server you build
yourself.
