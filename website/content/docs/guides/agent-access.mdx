...
<Callout type="info">
  Agent tokens are a **machine-to-machine** authentication system. They work
  alongside your existing admin token — nothing changes for dashboard sign-in.
</Callout>
...
Agent tokens can only be created by the admin. There is no dashboard UI yet — use the API directly with `curl` or any HTTP client.
