...
import { Kbd } from "@/dashboard/components/ui/kbd"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { demoRoute, isDemoMode } from "@/dashboard/lib/demo"
import { LogoMark } from "@/dashboard/components/logo-mark"
import { cleanVersion } from "@/dashboard/lib/utils"
import { toastManager } from "@/dashboard/components/ui/toast"
import {
  CommandDialog,
  CommandDialogPopup,
  Command,
...
import { Tooltip, TooltipTrigger, TooltipContent } from "@/dashboard/components/ui/tooltip"
import {
...
export function AppShell({ children, appCount, hasActiveLogs }: AppShellProps) {
  ...
  const isActive = (href: string) => {
    const target = demoRoute(href, pathname)
    ...
    <SidebarProvider className="h-screen overflow-hidden bg-sidebar">
      <div className="relative flex h-screen w-full overflow-hidden bg-sidebar text-foreground transition-colors duration-200 selection:bg-primary/20">
        ...
        <SidebarInset className="du-card relative z-10 m-0 md:m-2 md:ml-0 flex min-w-0 flex-1 flex-col overflow-hidden rounded-xl border bg-background text-card-foreground shadow-xs/5">
          {isDemoMode() && (
            <div className="shrink-0 flex flex-wrap items-center justify-between gap-2 border-b border-primary/20 bg-primary/8 px-4 py-2 text-xs text-foreground">
              <span>
              ...
              <a
                href="https://better-paas.com/docs/quickstart"
                target="_blank"
...
// (`import { StatusDot } from "@/dashboard/components/app-shell"`) keep working while the
// implementation lives in one place.
export { StatusDot } from "@/dashboard/components/status-badge"
