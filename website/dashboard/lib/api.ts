...
import { demoReq, demoUploadReq } from "./demo-api"
...
export function getApiBase(): string {
  if (isDemoMode()) return `https://${DEMO_DOMAIN}`
...
async function req<T>(path: string, options?: RequestInit): Promise<T> {
  if (isDemoMode()) return demoReq<T>(path, options)
...
async function uploadReq<T>(path: string, formData: FormData): Promise<T> {
  if (isDemoMode()) return demoUploadReq<T>(path, formData)
...
export const authApi = {
  ...
  verify: (token: string) => {
    ...
    return req<{ valid: boolean }>("/api/auth/verify", {
      method: "POST",
...
export const api = {
  ...
  backups: {
    ...
    downloadUrl: (name: string) => {
      if (isDemoMode()) {
        return `#demo-backup-${encodeURIComponent(name)}`
      }
      return `${getApiBase()}/api/backups/download?name=${encodeURIComponent(name)}`
    },
  ...
  analytics: {
    ...
    scriptUrl: () => `${getApiBase()}/api/analytics/script.js`,
    // The one-line snippet the operator pastes into their deployed site.
    snippet: (appId: string) =>
      `<script defer data-site="${appId}" src="${getApiBase()}/api/analytics/script.js"></script>`,
    ...
    installPrompt: (appId: string) => buildInstallPrompt(appId, getApiBase()),
  },
}
...
async function createWs(path: string): Promise<WebSocket> {
  if (isDemoMode()) return createDemoWebSocket(path)
